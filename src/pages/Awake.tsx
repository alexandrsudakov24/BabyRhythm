  ...
  return (
    <Layout title={t("awake.title")}>
      <div className="flex flex-col items-center gap-6 py-8">
        <BigButton active={!!active} color="awake" onClick={active ? stop : start}>
          <span className="text-3xl">{active ? "☀️" : "😴"}</span>
          <span className="text-xs font-semibold">{active ? t("awake.stop") : t("awake.start")}</span>
        </BigButton>
        {active && (
          <div className="w-full max-w-xs flex flex-col items-center gap-3">
            <div className="text-center">
              <p className="text-xs text-white/60 mb-2">{t("awake.duration")}</p>
      ...
      <h2 className="text-xs font-semibold text-white/60 mb-3 uppercase tracking-wider">{t("awake.history")}</h2>
