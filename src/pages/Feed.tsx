    ...
    <Layout title={t('feed.title')}>
      ...
      <div className="flex justify-center py-8">
        <button
          ...
          className="w-36 h-36 rounded-full bg-feed/10 text-feed flex flex-col items-center
                     justify-center gap-1 text-sm font-semibold active:scale-95 transition-transform"
        >
      ...
      {sheet && (
        <div className="fixed inset-0 z-50 flex items-end">
          ...
          <div className="relative w-full bg-slate-900 rounded-t-3xl px-5 pt-5 pb-10 flex flex-col gap-4">
              ...
              {FEED_TYPES.map(ft => (
                <button
                  ...
                  className={`py-3 rounded-xl text-sm font-medium transition-colors
                    ${feedType === ft ? 'bg-feed text-white' : 'bg-slate-800 text-white/60'}`}
                >
                  {t(`feed.type.${ft}`)}
                </button>
              ))}
